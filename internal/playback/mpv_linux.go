...
func mpvCommand(handle uintptr, args []string) error {
	...
	ret, _, _ := purego.SyscallN(mpv_command, handle, uintptr(unsafe.Pointer(&cArgs[0])))
...
func mpvSetFlag(handle uintptr, name string, val bool) error {
	...
	ret, _, _ := purego.SyscallN(mpv_set_property, handle, uintptr(unsafe.Pointer(&nameBytes[0])), 3, uintptr(unsafe.Pointer(&cVal))) // 3 is MPV_FORMAT_FLAG
...
func mpvSetFloat(handle uintptr, name string, val float64) error {
	nameBytes := append([]byte(name), 0)
	ret, _, _ := purego.SyscallN(mpv_set_property, handle, uintptr(unsafe.Pointer(&nameBytes[0])), 5, uintptr(unsafe.Pointer(&val))) // 5 is MPV_FORMAT_DOUBLE
	runtime.KeepAlive(nameBytes)
...
func mpvSetOptionString(handle uintptr, name string, val string) error {
	...
	ret, _, _ := purego.SyscallN(mpv_set_option_string, handle, uintptr(unsafe.Pointer(&nameBytes[0])), uintptr(unsafe.Pointer(&valBytes[0])))
	runtime.KeepAlive(nameBytes)
